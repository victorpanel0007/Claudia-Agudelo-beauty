...
export default function ReportesView() {
  ...
  async function loadReports() {
    ...
    const [daily, weekly, monthly, services, specialists, clients] = await Promise.all([
      ...
      supabase.from('citas').select('valor_final').gte('fecha_inicio', weekStart.toISOString()).eq('estado', 'completada'),
  ...
  const ventas = [
    ...
    { label: 'Ventas mes', value: data.ventasMensuales, icon: TrendingUp, color: 'text-beauty-gold bg-beauty-rose-light' },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h2 className="text-xl font-bold text-beauty-black">Reportes</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {ventas.map(v => (
          <div key={v.label} className="beauty-card p-5">
            ...
            <p className="text-xl font-bold text-beauty-black">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="beauty-card">
          <div className="p-4 border-b border-gray-100">
            <div className="flex items-center gap-2">
              <Scissors size={18} className="text-beauty-gold" />
              <h3 className="font-semibold text-beauty-black text-sm">Servicios más vendidos</h3>
            </div>
          </div>
          <div className="p-4 space-y-3">
            {data.serviciosMasVendidos.length === 0 ? (
              ...
              <div key={i} className="flex items-center gap-3">
                <span className="text-beauty-gold font-bold text-sm w-5">{i + 1}.</span>
                <div className="flex-1 min-w-0">
                  <p className="text-beauty-black text-sm truncate">{s.nombre}</p>
                  <div className="mt-1 bg-gray-100 rounded-full h-1.5">
                    <div
                      className="bg-beauty-gold h-1.5 rounded-full"
        ...
        <div className="beauty-card">
          <div className="p-4 border-b border-gray-100">
            <div className="flex items-center gap-2">
              <Award size={18} className="text-beauty-gold" />
              <h3 className="font-semibold text-beauty-black text-sm">Especialista top</h3>
            </div>
          </div>
          <div className="p-4 space-y-3">
            {data.especialistaTopCitas.length === 0 ? (
              ...
              <div key={i} className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-beauty-rose-light flex items-center justify-center">
                  <span className="text-beauty-gold font-bold text-xs">{e.nombre.charAt(0)}</span>
                </div>
                <div className="flex-1">
                  <p className="text-beauty-black text-sm">{e.nombre}</p>
                </div>
                <span className="bg-beauty-gold/20 text-beauty-gold-dark text-xs font-medium px-2 py-0.5 rounded-full">
                  {e.count} citas
                </span>
              </div>
            ))}
        ...
        <div className="beauty-card">
          <div className="p-4 border-b border-gray-100">
            <div className="flex items-center gap-2">
              <Users size={18} className="text-beauty-gold" />
              <h3 className="font-semibold text-beauty-black text-sm">Clientes frecuentes</h3>
            </div>
          </div>
          <div className="p-4 space-y-3">
            {data.clientesFrecuentes.length === 0 ? (
              ...
              <div key={i} className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-beauty-rose-light flex items-center justify-center">
                  <span className="text-beauty-gold font-bold text-xs">{c.nombre.charAt(0)}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-beauty-black text-sm truncate">{c.nombre}</p>
